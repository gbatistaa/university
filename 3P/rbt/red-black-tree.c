...
void inicializarArvore(tipoArvore *arvore) { arvore->raiz = NULL; }
...
void rotacaoDireita(tipoArvore *arvore, tipoNo *no) {
  ...
  //  Se o filho direito de esq (filho direito do filho esquerdo de nó
  //  rotacionado) existir, atualize seu pai para nó:
  if (esq->dir != NULL) {
    esq->dir->pai = no;
  }
  ...
  if (no->pai == NULL) {
    ...
    no->pai->esq =
        esq; // Se nó era o filho esquerdo, agora o filho esquerdo será esq
  } else {
    no->pai->dir =
        esq; // Se nó era o filho direito, agora o filho direito será esq
  }
...
void removerNo(tipoArvore *arvore, int valor) {
  ...
  tipoNo *substituto;
  int corOriginal =
      noRemovido->cor; // Armazenar a cor original para verificação posterior
